...
const RULES: Array<{ cap: CapabilityId; kws: RegExp[] }> = [
  ...
  // Phase 2 · Batch 3 — mutates first
  { cap: 'fm.listing.create',   kws: [/list.*(item|on market)|create.*listing|publish.*listing/i, /העלה למרקט|פרסם.*מודעה|תפרסם.*מוצר|פרסם גיג/] },
  { cap: 'message.send',        kws: [/send.*message|reply to/i, /שלח הודעה|תשלח לו|תענה ל/] },
  { cap: 'subscription.portal', kws: [/manage.*(subscription|plan|billing)|cancel.*subscription/i, /נהל.*מנוי|בטל מנוי|חיוב|פורטל מנוי/] },
  { cap: 'checkout.create',     kws: [/upgrade|subscribe|checkout|buy.*plan/i, /שדרג|הרשם.*פלוס|הרשם.*אפקס|שלם|תשלום על המנוי/] },
  { cap: 'work.startSession',   kws: [/start.*(focus|deep work|session|timer)/i, /התחל.*פוקוס|פתח טיימר|זמן עבודה|התחל סשן/] },
  { cap: 'schedule.block',      kws: [/schedule.*block|block.*time|put.*calendar/i, /קבע בלוק|חסום זמן|תוסיף ליומן|הוסף בלוק/] },
  { cap: 'tts.speak',           kws: [/read.*aloud|speak.*this|say.*it/i, /הקרא לי|תקרא לי|דבר את זה|השמע/] },
  { cap: 'voice.transcribe',    kws: [/transcribe|voice note/i, /תמלל|הקלטה קולית/] },
  // Reads
  { cap: 'fm.listing.preview',  kws: [/show.*listing|preview.*listing/i, /הצג מודעה|תראה לי את ה?מודעה/] },
  { cap: 'fm.search',           kws: [/marketplace|free market|bounties|gigs/i, /שוק חופשי|באונטיז|מרקט|מודעות|גיגים/] },
  { cap: 'wallet.status',       kws: [/wallet|balance|\bmos\b/i, /ארנק|יתרה|כמה MOS|כמה מוס/] },
  { cap: 'wallet.open',         kws: [/open.*wallet/i, /פתח.*ארנק/] },
  { cap: 'community.thread',    kws: [/open.*thread|open.*discussion/i, /פתח שיחה|פתח דיון/] },
  { cap: 'community.feed',      kws: [/community|feed/i, /קהילה|פיד/] },
  { cap: 'message.search',      kws: [/search.*messages|find.*messages|my messages/i, /חפש בהודעות|ההודעות שלי|הודעות שלי/] },
  { cap: 'subscription.status', kws: [/my.*subscription|plan.*status|subscription status/i, /המנוי שלי|מצב מנוי|סטטוס מנוי/] },
  { cap: 'work.summarize',      kws: [/work.*today|focus.*summary|how.*much.*work/i, /כמה עבדתי|סיכום עבודה|פוקוס היום|עבודה היום/] },
  // Reads (no mutation)
