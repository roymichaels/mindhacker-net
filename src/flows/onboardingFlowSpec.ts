  ...
  steps: [
    ...
    // ─── Step 5: Sleep Architecture ───
    {
      ...
      miniSteps: [
        ...
        {
          id: 'morning_routine_desire',
          title_he: 'כמה זמן בוקר אישי אתה רוצה לפני שמתחיל יום העבודה?',
          title_en: 'How much personal morning time do you want before work starts?',
          inputType: 'single_select',
          options: [
            { value: 'none', label_he: 'לא צריך', label_en: 'None needed', icon: '⏩' },
            { value: '15_30', label_he: '15-30 דקות', label_en: '15-30 min', icon: '🕐' },
            { value: '30_60', label_he: '30-60 דקות', label_en: '30-60 min', icon: '🕑' },
            { value: '60_90', label_he: '60-90 דקות', label_en: '60-90 min', icon: '🕒' },
            { value: '90_plus', label_he: '90+ דקות', label_en: '90+ min', icon: '🌅' },
          ],
          validation: { required: true },
          dbPath: { table: 'launchpad_progress', column: 'step_2_profile_data', jsonPath: 'morning_routine_desire' },
        },
      ],
    },
    ...
    // ─── Step 9: Work Reality ───
    {
      ...
      miniSteps: [
        ...
        {
          id: 'work_engagement_style',
          title_he: 'מה הסגנון שבו אתה עובד ביום רגיל?',
          title_en: 'What best describes your daily work style?',
          inputType: 'single_select',
          options: [
            { value: 'deep_blocks', label_he: 'בלוקים ממוקדים — יושב ועובד שעות ברצף', label_en: 'Focused blocks — hours of continuous work', icon: '🎯' },
            { value: 'scattered', label_he: 'מפוזר — פגישות, שיחות, הודעות לאורך היום', label_en: 'Scattered — meetings, calls, messages throughout', icon: '📱' },
            { value: 'on_call', label_he: 'זמינות — צריך להיות זמין אבל לא עובד ברציפות', label_en: 'On-call — need to be available but not working continuously', icon: '📡' },
            { value: 'mixed', label_he: 'שילוב — חלק ממוקד, חלק זמינות', label_en: 'Mixed — some focused, some on-call', icon: '🔄' },
            { value: 'minimal', label_he: 'מינימלי — עבודה קלה או לא עובד', label_en: 'Minimal — light work or not working', icon: '🌿' },
          ],
          validation: { required: true },
          dbPath: { table: 'launchpad_progress', column: 'step_2_profile_data', jsonPath: 'work_engagement_style' },
        },
        {
          id: 'availability_window',
          title_he: 'כמה שעות ביום אתה חייב להיות זמין בכלל? (כולל on-call, לא רק עבודה ממוקדת)',
          title_en: 'How many hours a day must you be available overall? (including on-call, not just focused work)',
          inputType: 'single_select',
          options: [
            { value: 'none', label_he: 'אין מחויבות קבועה', label_en: 'No fixed commitment', icon: '🟢' },
            ...
            { value: '12_plus', label_he: '12+ שעות (כמעט כל היום)', label_en: '12+ hours (almost all day)', icon: '🔴' },
          ],
          validation: { required: true },
          dbPath: { table: 'launchpad_progress', column: 'step_2_profile_data', jsonPath: 'availability_window' },
        },
        {
          ...
          options: [
            ...
            { value: 'dont_know', label_he: 'לא יודע — תעזור לי למצוא', label_en: 'Not sure — help me find out', icon: '🤖' },
          ],
          validation: { required: true },
          dbPath: { table: 'launchpad_progress', column: 'step_2_profile_data', jsonPath: 'free_blocks_available' },
        },
        {
          id: 'side_projects',
        ...
        },
        {
          ...
          validation: { required: true },
          dbPath: { table: 'launchpad_progress', column: 'step_2_profile_data', jsonPath: 'work_start_time' },
        },
        {
          id: 'work_end_time',
          ...
          validation: { required: true },
          dbPath: { table: 'launchpad_progress', column: 'step_2_profile_data', jsonPath: 'work_end_time' },
        },
        {
          id: 'commute_duration',
    ...
    // ─── Step 10: Life Load ───
    {
      ...
      miniSteps: [
        ...
        },
        {
          ...
          title_he: 'מה מצב הזוגיות שלך?',
          title_en: 'What is your relationship status?',
          inputType: 'single_select',
          options: [
            { value: 'single', label_he: 'רווק/ה', label_en: 'Single', icon: '🙋' },
            { value: 'in_relationship', label_he: 'בזוגיות', label_en: 'In a relationship', icon: '💑' },
            { value: 'married', label_he: 'נשוי/אה', label_en: 'Married', icon: '💍' },
            { value: 'divorced_separated', label_he: 'גרוש/ה / נפרד/ת', label_en: 'Divorced / Separated', icon: '💔' },
            { value: 'complicated', label_he: 'מסובך', label_en: 'Complicated', icon: '🤷' },
          ],
          validation: { required: true },
          dbPath: { table: 'launchpad_progress', column: 'step_2_profile_data', jsonPath: 'relationship_status' },
        },
      ],
    },
      ...
      miniSteps: [
        {
          ...
          options: [
            ...
            { value: 'water_sports', label_he: 'ספורט ימי', label_en: 'Water sports', icon: '🏄' },
            { value: 'cycling', label_he: 'אופניים', label_en: 'Cycling', icon: '🚴' },
            ...
            { value: 'gym', label_he: 'חדר כושר', label_en: 'Gym', icon: '💪' },
            { value: 'martial_arts', label_he: 'אומנויות לחימה', label_en: 'Martial arts', icon: '🥋' },
            { value: 'gaming', label_he: 'גיימינג', label_en: 'Gaming', icon: '🎮' },
            { value: 'music', label_he: 'מוזיקה', label_en: 'Music', icon: '🎵' },
            { value: 'singing', label_he: 'שירה', label_en: 'Singing', icon: '🎤' },
            { value: 'cooking', label_he: 'בישול', label_en: 'Cooking', icon: '👨‍🍳' },
            { value: 'baking', label_he: 'אפייה', label_en: 'Baking', icon: '🧁' },
            ...
            { value: 'diy', label_he: 'DIY / בנייה', label_en: 'DIY', icon: '🔨' },
            { value: 'social_events', label_he: 'אירועים חברתיים', label_en: 'Social events', icon: '🎉' },
            { value: 'movies', label_he: 'סרטים / סדרות', label_en: 'Movies / TV', icon: '🎬' },
            { value: 'podcasts', label_he: 'פודקאסטים', label_en: 'Podcasts', icon: '🎧' },
            { value: 'extreme', label_he: 'ספורט אתגרי', label_en: 'Extreme sports', icon: '🧗' },
            { value: 'board_games', label_he: 'משחקי קופסה', label_en: 'Board games', icon: '♟️' },
            { value: 'volunteering', label_he: 'התנדבות', label_en: 'Volunteering', icon: '🤝' },
            { value: 'meditation', label_he: 'מדיטציה', label_en: 'Meditation', icon: '🧘‍♂️' },
            { value: 'fishing', label_he: 'דיג', label_en: 'Fishing', icon: '🎣' },
            { value: 'pets', label_he: 'חיות מחמד', label_en: 'Pets', icon: '🐕' },
            { value: 'fashion', label_he: 'אופנה / סטיילינג', label_en: 'Fashion', icon: '👗' },
            { value: 'coding', label_he: 'תכנות', label_en: 'Coding', icon: '💻' },
            { value: 'content_creation', label_he: 'יצירת תוכן', label_en: 'Content creation', icon: '📹' },
            { value: 'motorsports', label_he: 'מוטורספורט', label_en: 'Motorsports', icon: '🏍️' },
            { value: 'skateboarding', label_he: 'סקייטבורד', label_en: 'Skateboarding', icon: '🛹' },
          ],
    ...
    // ─── Step 11: Execution Pattern ───
    {
      ...
      miniSteps: [
        ...
        {
          id: 'previous_change_attempts',
          title_he: 'כמה פעמים ניסית לשנות הרגלים ב-12 חודשים האחרונים?',
          title_en: 'How many times have you tried to change habits in the last 12 months?',
          inputType: 'single_select',
          options: [
            ...
            { value: '6_plus', label_he: '6+ פעמים', label_en: '6+ times', icon: '🟠' },
            { value: 'lost_count', label_he: 'הפסקתי לספור', label_en: 'Lost count', icon: '🔴' },
          ],
          validation: { required: true },
          dbPath: { table: 'launchpad_progress', column: 'step_2_profile_data', jsonPath: 'previous_change_attempts' },
        },
      ],
    },
    ...
    // ─── Step 12: Friction Trigger ───
    {
      ...
      miniSteps: [
        ...
        {
          id: 'stress_default_behavior',
          title_he: 'כשאתה בלחץ, מה הדבר הראשון שאתה עושה?',
          title_en: 'When stressed, what is the first thing you do?',
          inputType: 'multi_select',
          options: [
            { value: 'eat', label_he: 'אוכל', label_en: 'Eat', icon: '🍔' },
            { value: 'scroll_phone', label_he: 'גולל בטלפון', label_en: 'Scroll phone', icon: '📱' },
            { value: 'smoke', label_he: 'מעשן', label_en: 'Smoke', icon: '🚬' },
            { value: 'substances', label_he: 'שימוש בחומרים (קנאביס, אלכוהול וכו׳)', label_en: 'Substances (cannabis, alcohol, etc.)', icon: '🍷' },
            { value: 'partying', label_he: 'יוצא למסיבות / בילויים', label_en: 'Partying / going out', icon: '🎉' },
            { value: 'sleep', label_he: 'הולך לישון', label_en: 'Sleep', icon: '😴' },
            { value: 'isolate', label_he: 'מתבודד', label_en: 'Isolate', icon: '🏠' },
            { value: 'argue', label_he: 'נכנס לעימותים', label_en: 'Argue', icon: '💥' },
            { value: 'work_more', label_he: 'עובד יותר', label_en: 'Work more', icon: '💻' },
            { value: 'exercise', label_he: 'מתאמן', label_en: 'Exercise', icon: '💪' },
          ],
          validation: { required: true },
          dbPath: { table: 'launchpad_progress', column: 'step_2_profile_data', jsonPath: 'stress_default_behavior' },
        },
      ],
    },
    ...
    // ─── Step 14: 90-Day Vector ───
    {
      ...
      miniSteps: [
        ...
        },
        {
          ...
          title_he: 'מה הדבר שהכי מגביל אותך היום ואתה לא יכול לשנות אותו כרגע?',
          title_en: 'What is the biggest thing limiting you right now that you can\'t change?',
          prompt_he: 'גורם שצריך לעבוד סביבו, לא נגדו',
          prompt_en: 'A factor you need to work around, not against',
          inputType: 'multi_select',
          options: [
            { value: 'time', label_he: 'חוסר זמן', label_en: 'Lack of time', icon: '⏰' },
            { value: 'money', label_he: 'מצב כלכלי', label_en: 'Financial situation', icon: '💸' },
            { value: 'family', label_he: 'מחויבויות משפחתיות', label_en: 'Family obligations', icon: '👨‍👩‍👧' },
            { value: 'mental_state', label_he: 'מצב נפשי / רגשי', label_en: 'Mental / emotional state', icon: '🧠' },
            { value: 'health', label_he: 'מגבלה בריאותית', label_en: 'Health limitation', icon: '🏥' },
            { value: 'environment', label_he: 'סביבה / מיקום גיאוגרפי', label_en: 'Environment / location', icon: '🏠' },
            { value: 'relationships', label_he: 'מערכות יחסים', label_en: 'Relationships', icon: '💔' },
            { value: 'energy', label_he: 'אנרגיה / עייפות כרונית', label_en: 'Energy / chronic fatigue', icon: '🔋' },
            { value: 'none', label_he: 'אין מגבלה משמעותית', label_en: 'No significant constraint', icon: '✅' },
          ],
          validation: { required: true, minSelected: 1 },
