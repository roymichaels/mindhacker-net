...
import type { PractitionerService } from '@/hooks/usePractitioners';
import { cn } from '@/lib/utils';
import { format, addDays, isBefore, startOfDay } from 'date-fns';
import { toast } from 'sonner';

interface PractitionerBookingViewProps {
...
const PractitionerBookingView = ({
  practitionerId,
  services,
  practitionerName,
  onBack,
  preSelectedServiceId,
}: PractitionerBookingViewProps) => {
  const { t, isRTL, language } = useTranslation();
  const queryClient = useQueryClient();
  const ArrowBack = isRTL ? ArrowRight : ArrowLeft;

  const [step, setStep] = useState<Step>(preSelectedServiceId ? 'date' : 'service');
  const [selectedService, setSelectedService] = useState<PractitionerService | null>(
...
export default PractitionerBookingView;
