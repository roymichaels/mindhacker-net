/**
 * StrategyRoadmapSidebar — Left sidebar for Strategy page.
 * Shows the full 100-day plan as a 10-phase vertical timeline.
 */
...
import {
  ...
  Trophy, PanelRightClose, PanelRightOpen,
  Loader2, Target,
} from 'lucide-react';
...
export function StrategyRoadmapSidebar() {
  ...
  return (
    <aside className={cn(
      ...
      {/* COLLAPSED */}
      {collapsed && (
        <div className="flex flex-col items-center justify-between h-full pt-10 pb-3">
          <div className="flex flex-col items-center gap-1 mt-2">
            ...
            <span className="text-[9px] font-bold text-primary mt-1">{progressPct}%</span>
          </div>
        </div>
      )}
                            ...
                            <div className="ps-7 pe-1 py-1 space-y-0.5">
                              {g.milestones.map((m) => (
                                <div
                                  key={m.id}
                                  className={cn(
                                    "flex items-start gap-2 p-1.5 rounded-md text-start transition-colors",
                                    m.is_completed ? "opacity-60" : "hover:bg-muted/30"
                                  )}
                                >
                                  {m.is_completed ? (
