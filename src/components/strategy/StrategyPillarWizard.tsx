...
export function StrategyPillarWizard({ open, onOpenChange, onPlanGenerated }: StrategyPillarWizardProps) {
  ...
  const handlePillarClick = (domain: LifeDomain) => {
    ...
    if (isSelected && !isAssessed) {
      // Already selected but not assessed — open assessment
      setAssessingDomain(domain.id);
    } else if (isSelected && isAssessed) {
      // Already assessed — allow deselect or re-assess
      setAssessingDomain(domain.id);
    } else {
      // Not selected — toggle selection, then open assessment
            ...
            {CORE_DOMAINS.map((domain, i) => {
              ...
              return (
                <motion.button
                  ...
                  className={cn(
                    'flex flex-col items-center gap-1.5 rounded-xl border bg-gradient-to-br p-3 text-center transition-all relative',
                    isSelected
