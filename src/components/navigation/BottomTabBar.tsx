...
const TAB_COLORS: Record<string, { solid: string; text: string; inactive: string }> = {
  ...
  strategy: { solid: 'bg-cyan-500', text: 'text-white', inactive: 'text-cyan-400/60' },
  hypnosis: { solid: 'bg-fuchsia-500', text: 'text-white', inactive: 'text-fuchsia-400/60' },
    ...
    <nav className="fixed bottom-0 inset-x-0 z-50 border-t border-border bg-background/100">
      <div className="flex items-center justify-around h-[84px] px-2">
        {tabs.map((tab) => {
          ...
          const Icon = tab.icon;
          const colors = TAB_COLORS[tab.id] || TAB_COLORS.strategy;
