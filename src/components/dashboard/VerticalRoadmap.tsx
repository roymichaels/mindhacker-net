...
function HorizontalTimeline({ months, milestones, currentWeek, progressPercent, isHe }: TimelineProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        <div className="relative flex items-start gap-0" style={{ minWidth: `${milestones.length * 76}px` }}>
          {/* Horizontal line background */}
          <div className="absolute top-[14px] left-0 right-0 h-[3px] bg-muted/30 rounded-full" />
          {/* Horizontal line progress fill */}
          <div
            className="absolute top-[14px] left-0 h-[3px] rounded-full bg-gradient-to-r from-emerald-500 via-amber-500 to-amber-500/30 transition-all duration-700"
          ...
          {months.map((month, monthIdx) => (
            <div key={monthIdx} className="flex items-start">
              {/* Month divider pill */}
              {monthIdx > 0 && (
                <div className="flex flex-col items-center justify-start pt-0 mx-1 snap-center" style={{ minWidth: '40px' }}>
                  ...
                  <div className="px-2 py-0.5 rounded-full bg-amber-500/10 backdrop-blur-md border border-amber-500/20">
                    <span className="text-[8px] font-bold text-amber-500 whitespace-nowrap">
              ...
              {month.weeks.map((milestone) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Title */}
                    <p className={cn(
                      "text-[9px] leading-tight text-center max-w-[68px] mt-0.5",
                    ...
                    {/* Current indicator */}
                    {isCurrent && (
                      <span className="mt-1 text-[7px] font-bold px-1.5 py-0.5 rounded-full bg-amber-500/20 text-amber-500 border border-amber-500/30">
                        {isHe ? 'כאן' : 'HERE'}
                      </span>
                    )}
                  </div>
