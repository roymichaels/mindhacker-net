...
export const MyRecordings = () => {
  ...
  const { data: pendingPurchases, isLoading: loadingPending } = useQuery({ queryKey: ["my-pending-audio-purchases"], queryFn: async () => { const { data: user } = await supabase.auth.getUser(); if (!user.user) throw new Error("Not authenticated"); const { data, error } = await supabase.from("content_purchases").select(`id, purchase_date, content_products!content_purchases_product_id_fkey (title, slug)`).eq("user_id", user.user.id).eq("payment_status", "completed").is("access_granted_at", null); if (error) throw error; const filtered = (data || []).filter((p: any) => p.content_products?.slug?.includes("personal-hypnosis")); return filtered as unknown as PendingPurchase[]; } });
