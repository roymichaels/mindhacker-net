/**
 * @tab Coaches
 * @purpose Detail view for a single coach — bio, services, reviews, booking
...
import { useCoach, type Coach } from '@/domain/coaches';
import { useQuery } from '@tanstack/react-query';
import { supabase } from '@/integrations/supabase/client';
import CoachMiniItemCard, { type MiniItemCardData } from './CoachMiniItemCard';
import CoachReviewSlider from './CoachReviewSlider';
import CoachBookingView from './CoachBookingView';
...
const CoachDetailView = ({ practitioner: basicPractitioner, onBack }: CoachDetailViewProps) => {
  const { t, isRTL, language } = useTranslation();
  const { data: practitioner, isLoading } = useCoach(basicPractitioner.slug);
    ...
    <div className="space-y-5">
      ...
      {practitioner && (
        <div>
          ...
          <CoachReviewSlider reviews={practitioner.reviews || []} testimonials={testimonials || []} />
        </div>
      )}
    </div>
  );
