...
const MILESTONES: EarnMilestone[] = [
  ...
  { id: 'first_mine', index: 3, titleEn: 'Mine Your First MOS', titleHe: 'כרה את ה-MOS הראשון שלך', descEn: 'Complete a daily activity to mine tokens', descHe: 'השלם פעילות יומית כדי לכרות טוקנים', icon: <Zap className="w-6 h-6" />, action: 'auto', rewardMos: 10 },
  ...
  { id: 'share_link', index: 5, titleEn: 'Share Your Link', titleHe: 'שתף את הקישור שלך', descEn: 'Copy and share your referral link', descHe: 'העתק ושתף את קישור ההפניה שלך', icon: <UserPlus className="w-6 h-6" />, action: 'auto', rewardMos: 10 },
...
export function EarnLaunchpadModal({ open, onOpenChange }: EarnLaunchpadModalProps) {
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-lg p-0 gap-0 overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        {isLoading ? (
          ...
          <div className="space-y-4 p-5">
            ...
            <div className="text-center space-y-1">
              <h2 className="text-lg font-black text-foreground flex items-center justify-center gap-2 tracking-tight">
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden">
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isActive ? 'w-5 h-2 bg-primary' : done ? 'w-2 h-2 bg-primary/40' : 'w-2 h-2 bg-border'
                        )}
                      />
                    );
                  })}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={cn(
                        ...
                        {isCompleted ? <CheckCircle2 className="w-6 h-6" /> : isLocked ? <Lock className="w-5 h-5" /> : currentMilestone.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-0.5">
                          ...
                          <span className={cn('text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0', isCompleted ? 'bg-primary/10 text-primary' : 'bg-amber-500/10 text-amber-500')}>
                    ...
                    {isLocked && (
                      <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/50 rounded-lg px-3 py-2">
                        <Lock className="w-4 h-4" /> {isHe ? 'השלם את אבן הדרך הקודמת כדי לפתוח' : 'Complete the previous milestone to unlock'}
                      </div>
                    )}
              ...
              <div className="flex items-center justify-between px-4 pb-3 pt-1">
                <Button variant="ghost" size="sm" className="gap-1 text-xs" onClick={() => goTo(activeMilestoneIdx - 1)} disabled={activeMilestoneIdx === 0}>
                  <ChevronLeft className="w-4 h-4" /> {isHe ? 'הקודם' : 'Prev'}
                </Button>
                <Button variant="ghost" size="sm" className="gap-1 text-xs" onClick={() => goTo(activeMilestoneIdx + 1)} disabled={activeMilestoneIdx === MILESTONES.length - 1}>
                  {isHe ? 'הבא' : 'Next'} <ChevronRight className="w-4 h-4" />
                </Button>
              </div>
            </div>
