...
const HeroPortraitEffect = ({ portraitUrl, alt }: HeroPortraitEffectProps) => {
  ...
  // Both light and dark modes get effects, with adapted colors
  return (
    <div className="relative mx-auto mb-4 sm:mb-6 md:mb-8 animate-fade-in" style={{ animationFillMode: 'forwards' }}>
      <div 
        className="relative w-28 h-28 sm:w-36 sm:h-36 md:w-48 md:h-48 mx-auto"
        style={{ animation: `float-gentle ${animationDuration.float} ease-in-out infinite` }}
      >
        {/* Glow effect behind image - works in both modes */}
        <div 
          className="absolute inset-0 rounded-full blur-2xl scale-150 dark:opacity-100 opacity-40"
          style={{ 
        ...
        <img 
          src={portraitUrl} 
          alt={alt}
          className="relative w-full h-full object-cover rounded-full border-2 border-primary/30 dark:border-transparent shadow-lg dark:shadow-none"
          style={{
        ...
        {/* Pulsing glow rings - both modes with adapted opacity */}
        <div 
          className="absolute inset-0 rounded-full border-2 sm:border-3 border-primary/40 dark:border-primary/60 pointer-events-none"
          style={{ 
        ...
        />
        <div 
          className="absolute -inset-2 sm:-inset-3 rounded-full border border-primary/25 dark:border-primary/30 pointer-events-none"
          style={{ 
            animation: `ring-pulse ${animationDuration.ring} ease-in-out infinite`,
            animationDelay: '0.5s',
        ...
        {/* Sparkle effects - both modes with adapted colors */}
        <Sparkle 
          className="absolute -top-2 -right-2 sm:-top-3 sm:-right-3 w-4 h-4 sm:w-5 sm:h-5 text-primary dark:text-primary"
          style={{ animation: `sparkle ${animationDuration.sparkle} ease-in-out infinite`, color: glowColor || undefined }}
        />
        <Sparkle 
          className="absolute top-1/4 -left-3 sm:-left-4 w-3 h-3 sm:w-4 sm:h-4 text-accent dark:text-accent"
          style={{ animation: `sparkle ${animationDuration.sparkle} ease-in-out infinite`, animationDelay: '0.3s' }}
        />
        <Sparkle 
          className="absolute -bottom-2 right-1/4 w-3 h-3 sm:w-4 sm:h-4 text-primary dark:text-primary"
          style={{ animation: `sparkle ${animationDuration.sparkle} ease-in-out infinite`, animationDelay: '0.6s', color: glowColor || undefined }}
        />
        <Sparkle 
          className="absolute top-0 left-1/3 w-2 h-2 sm:w-3 sm:h-3 text-primary/70 dark:text-secondary"
          style={{ animation: `sparkle ${animationDuration.sparkle} ease-in-out infinite`, animationDelay: '0.9s' }}
        />
        <Sparkle 
          className="absolute bottom-1/4 -right-3 sm:-right-4 w-3 h-3 sm:w-4 sm:h-4 text-accent dark:text-accent"
