/**
 * @page FocusHistory (/life/focus/history)
 * Lists all previous Focus assessments.
 */
...
import { ArrowLeft, ArrowRight, ChevronRight, ChevronLeft, Crosshair } from 'lucide-react';

export default function FocusHistory() {
  const navigate = useNavigate();
  const { t, isRTL } = useTranslation();
  const { config } = useFocusCoach();
      ...
      <div className="space-y-6 pb-8" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        {history.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-sm text-muted-foreground">{t('focus.noHistory')}</p>
          </div>
        ) : (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary" className="text-xs">
                      {t(`focus.confidence_${h.confidence}`)}
                    </Badge>
