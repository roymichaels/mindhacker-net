 ...
 * Entry point for Combat (לחימה) Warrior Capability Engine.
      ...
      <PageShell>
        <div className="space-y-6 pb-8" dir={isRTL ? 'rtl' : 'ltr'}>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            <Card className="p-8 text-center border-slate-500/20 bg-gradient-to-b from-slate-500/5 to-transparent">
              ...
              <div className="space-y-3 max-w-xs mx-auto">
                <div className={cn(
                  ...
                  vitalityComplete ? "border-emerald-500/30 bg-emerald-500/5" : "border-amber-500/30 bg-amber-500/5"
                )}>
                  <Sun className={cn("w-5 h-5", vitalityComplete ? "text-emerald-400" : "text-amber-400")} />
                  <span className="text-sm font-medium text-foreground flex-1 text-start">{t('combat.reqVitality')}</span>
                  {vitalityComplete
                ...
                <div className={cn(
                  "flex items-center gap-3 p-3 rounded-xl border",
                  focusComplete ? "border-emerald-500/30 bg-emerald-500/5" : "border-amber-500/30 bg-amber-500/5"
                )}>
                  <Crosshair className={cn("w-5 h-5", focusComplete ? "text-emerald-400" : "text-cyan-400")} />
                  <span className="text-sm font-medium text-foreground flex-1 text-start">{t('combat.reqFocus')}</span>
                  {focusComplete
                    ? <Check className="w-4 h-4 text-emerald-400" />
      ...
      <div className="space-y-6 pb-8" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        {latest ? (
          <>
            {/* Snapshot card */}
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
              <Card className="p-5 bg-gradient-to-b from-slate-500/10 to-transparent border-slate-500/30">
                ...
                <div className="flex gap-2">
                  <Button onClick={() => navigate('/life/combat/results')} className="flex-1 bg-slate-600 hover:bg-slate-700">
                    <BarChart3 className="w-4 h-4 me-1" /> {t('combat.viewResults')}
                  </Button>
                  <Button onClick={() => navigate('/life/combat/assess')} variant="outline" className="border-slate-500/40">
          ...
          /* No assessment yet */
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            <Card className="p-6 bg-gradient-to-b from-slate-500/10 to-transparent border-slate-500/30 text-center">
              <Swords className="w-10 h-10 text-slate-400 mx-auto mb-3" />
              ...
              <Button onClick={() => navigate('/life/combat/assess')} className="bg-slate-600 hover:bg-slate-700" size="lg">
