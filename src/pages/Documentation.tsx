...
export default function Documentation() {
  ...
  const abstractText = he
    ? `${brandName} הוא מערכת הפעלה אישית מבוססת בינה מלאכותית, המשלבת מנגנוני Play-to-Earn (P2E), נכסים דיגיטליים ייחודיים (Soul Avatar NFTs), גיימיפיקציה עמוקה, היפנוזה ומדיטציה מונחית AI (כולל סשנים אישיים מותאמים), מערכת למידה אדפטיבית, שוק פנימי (FreeMarket) עם 5 מסלולי קריירה מאוחדים (בעל עסק, מאמן, מטפל, יוצר תוכן, פרילנסר), התאמת מאמנים מבוססת AI, פלטפורמת מאמנים עם דפי נחיתה דינמיים וחנויות אישיות, הערכות עמודים מבוססות שיחת AI, תוכנית Consciousness Leap, בלוג Aurora Codex, מערכת אונבורדינג עם טקס כניסה, מערכת PWA מלאה, ניהול פלטפורמה מאובטח, תוכנית שותפים, ו-Aurora עם מודעות הקשרית מלאה — לתוך מערכת הפעלה אחת שעוטפת את חיי המשתמש. המסמך מציג את הארכיטקטורה, הכלכלה הדיגיטלית, מערכת ה-AI התודעתית (Aurora), מודל המנויים, ומפת הדרכים של הפרויקט. המטבע הפנימי MOS (100 MOS = $1.00) מבוסס על מנגנון Proof of Growth — מודל כריית נתונים שמתגמל פעילות אנושית אמיתית.`
    : `${brandName} is an AI-powered Personal Operating System that integrates Play-to-Earn (P2E) mechanics, unique digital assets (Soul Avatar NFTs), deep gamification, AI-guided hypnosis and meditation (including custom personal sessions), an adaptive learning system, an internal marketplace (FreeMarket) with 5 unified career paths (Business Owner, Coach, Therapist, Content Creator, Freelancer), AI-powered coach matching, a coach platform with dynamic landing pages and personal storefronts, chat-based pillar assessments, Consciousness Leap program, Aurora Codex blog, an onboarding system with initiation ceremony, full PWA support with home screen installation, secure platform management, an affiliate program, and Aurora with full contextual awareness — into a single operating system that wraps around the user's life. This paper presents the architecture, digital economy, consciousness AI engine (Aurora), subscription model, and roadmap. The internal currency MOS (100 MOS = $1.00) is based on a Proof of Growth mechanism — a data mining model that rewards genuine human activity.`;
