...
export default function FMWalletPage() {
  ...
  const { data: claims = [], isLoading: claimsLoading } = useFMClaims();
  ...
  // Split transactions into earn vs spend
    ...
    return (
      <div className="space-y-1.5">
        {txList.map((tx: any) => {
          ...
          return (
            <div key={tx.id} className="flex items-center gap-3 bg-card border border-border rounded-xl px-3.5 py-3 active:scale-[0.99] transition-transform">
  ...
  return (
    <div className="space-y-4 max-w-2xl mx-auto w-full py-4">
      {/* Balance Hero */}
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden bg-card border border-border rounded-2xl p-6">
