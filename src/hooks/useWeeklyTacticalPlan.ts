...
export function useWeeklyTacticalPlan(): PhasePlan & { isLoading: boolean; generateSchedule: () => Promise<void>; isGenerating: boolean } {
  ...
  // Fetch tactical schedules for ALL plans in the same batch (core + arena)
  const { data: aiSchedules, isLoading: scheduleLoading } = useQuery({
    queryKey: ['tactical-schedule', allPlanIds, currentPhase],
    queryFn: async () => {
  ...
  const toggleActionComplete = useCallback(async (action: TacticalAction) => {
    ...
    if (newDone) {
      ...
      if (prevData) {
        ...
        const normalizedTitleEn = normalize(action.titleEn || '');
        queryClient.setQueryData(prevKey, prevData.filter(item => {
          const itemTitle = normalize(item.title || '');
          const isMatch = (itemTitle === normalizedTitle || (normalizedTitleEn && itemTitle === normalizedTitleEn)) && item.scheduled_date === date;
          return !isMatch;
        }));
      }
    }
    ...
    try {
      if (newDone) {
        ...
        if (error) {
          console.warn('[toggle] upsert error, trying insert:', error.message);
        ...
        // Uncheck: update ALL matching done items for this user+date
        // Try Hebrew title
